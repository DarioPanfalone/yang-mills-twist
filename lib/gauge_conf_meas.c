...
void poly_sqr_for_tracedef(Gauge_Conf const * const GC,
                              Geometry const * const geo,
                              double* polysqr)
{
   long rsp;
   ...
   #ifdef OPENMP_MODE
   #pragma omp parallel for num_threads(NTHREADS) private(rsp) reduction(+: polysqr[:NCOLOR/2])
   #endif
   for(rsp=0; rsp<geo->d_space_vol; rsp++)
      {
      long r;
      ...
      one(&matrix2);
      double realp, imagp;
      for(k=0; k<(int)floor(NCOLOR/2.0); k++)
         {
         times_equal(&matrix2, &matrix);
         realp = NCOLOR * retr(&matrix2);
         imagp = NCOLOR * imtr(&matrix2);
         polysqr[k] += (realp * realp) + (imagp * imagp);
         }
      }

   for(j=0; j<(int)floor(NCOLOR/2.0); j++)
      {
      polysqr[j] *= geo->d_inv_space_vol;
      }
}
...
#if NCOLOR > 1
   double polysqr[NCOLOR / 2];
   poly_sqr_for_tracedef(GC, geo, polysqr);
   for(i=0; i<(int)floor(NCOLOR/2); i++)
      {
      fprintf(datafilep, "%.12g ", polysqr[i]);
      }
#endif
