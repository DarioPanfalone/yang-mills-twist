...
void real_main(char *in_file)
    ...
    if (NCOLOR != 3) {
        fprintf(stderr, "THIS PROGRAM COPUTES BARION ONLY FOR SU(#) !!! !!! !!!\n");
        exit(EXIT_FAILURE);
    }
    ...
    init_barion_aux_for_ml(&my_ptr, &geo, &param);
    alloc_polycorr_stuff(&GC, &geo, &param);
#else
    init_barion_aux(&my_ptr, &geo, &param);
#endif
    ...
    for(count=1; count < param.d_sample + 1; count++)
       ...
       if(count % param.d_measevery ==0 && count >= param.d_thermal)
         {
         #ifdef BARION_MULTILEVEL
         perform_measures_barion_onelevel_multiT(&GC, &geo, &param, datafilep, &my_ptr);
         #else
         perform_measures_barion_multihit(&GC, &geo, &param, datafilep, &my_ptr);
         #endif
         }
...
#if BARION_MULTILEVEL
    free_barion_aux_for_ml(&my_ptr, &geo, &param);
#else
    free_barion_aux(&my_ptr);
#endif
    // close data file
    fclose(datafilep);
    ...
    print_parameters_barion(&param, time1, time2);
...
void print_template_input(void)
  ...
  if(fp==NULL)
    ...
    fprintf(fp,"\n");
    fprintf(fp, "# for the barion meausres: \n");
    fprintf(fp, "triangle_base           2 # base of the triangle\n");
    fprintf(fp, "triangle_hight          2 # hight of the triangel (should be approx sqrt(3) / 2 * base)\n");
    fprintf(fp,"\n");
    fprintf(fp, "multihit   10\n"); // multihit
#ifdef BARION_MULTILEVEL
    fprintf(fp, "ml_upd     10\n");
    fprintf(fp, "ml_step     2\n");
#endif
    fprintf(fp, "\n");
