...
void real_main(char *in_file)
    ...
    for(count=2; count<STDIM; count++)
       ...
       if(tmp!= param.d_sizeg[count])
         {
         fprintf(stderr, "When using yang_mills_tracedef_tube all the spatial sizes have to be of equal length.\n");
         exit(EXIT_FAILURE);
         }
       }
    ...
    for(count=1; count < param.d_sample + 1; count++)
       ...
       if(count % param.d_measevery ==0 && count >= param.d_thermal)
         {
         perform_measures_localobs_with_tracedef(&GC, &geo, &param, datafilep, NULL);
         
         // preparation for the correlators
         compute_polyakov_tube_disc_highT(&GC, &geo, &tube_ptr);
         compute_plaquette_columns(&GC, &geo, &param, &tube_ptr);
         
         perform_measures_two_and_three_pts(GC.update_index, &geo, &param, &tube_ptr);
         perform_measures_plaqcor(GC.update_index, &geo, &tube_ptr);
         }
...
void print_template_input(void)
  ...
  if(fp==NULL)
    ...
    fprintf(fp, "#for flux tube\n");    
    fprintf(fp, "poly_dist_min           2  # minimum distance between the Polyakov loops\n");
    fprintf(fp, "poly_dist_max           3  # maximum distance between the Polyakov loops\n");
    fprintf(fp, "poly_dist_step          1  # increment of distance between the Polyakov loops\n");
    fprintf(fp, "plaq_dir               1 0 # plaquette orientation\n");
    fprintf(fp,"\n");
