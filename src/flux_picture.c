...
void real_main(char *in_file)
    ...
    fprintf(datafilep, "%s ", param.d_log_file);
    ...
    sources_cfg *src = new_sources_cfg(1, 1);
    for (int mu = 0; mu < STDIM; mu++) {
      src->position[mu][0] = 0;
      src->position[mu][1] = 0;
    }
    src->position[1][1] = param.d_dist_poly;    

    ape_smearing(&GC, &geo, 0.1, param.d_smearingstep);
    generic_fundsources_sweep(&GC, &geo, &param, src, datafilep);

    free_sources_cfg(src);
    time(&time2);
